...
// Inngest function to cancel booking and release seats if payment is not made within 10 minutes
const releaseSeatsAndDeleteBooking = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("check-payment-status", async () => {
      ...
      // Guard against missing booking
      if (!booking) {
        console.warn(`Booking not found: ${bookingId}`);
        return;
      }

      // Only release seats if not paid
      if (!booking.isPaid) {
        ...
        reminderTasks.map((task) =>
          sendEmail({
            ...
            body: `<div style="font-family: Arial, sans-serif; padding:20px;">
                        ...
                        <p>is scheduled to start at <strong>${new Date(
                          task.showTime
